...
# finding all prime numbers up to any given limit. It does so 
# by iteratively marking as composite (i.e. not prime) the 
...
# that have not been marked are prime. The wikipedia article 
...
class Sieve
  ...
  def primes
    while !@numbers.empty?
      ...
      @numbers.reject! { |num| num % @primes.last == 0 }
    end
    @primes
  end
end
